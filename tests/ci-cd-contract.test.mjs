...
const deployDocs = await readFile(new URL("../docs/deploy.md", import.meta.url), "utf8");
...
test("release publishes both process images with immutable provenance", () => {
  ...
  assert.match(release, /WEB_IMAGE: ghcr\.io\/canonical-cloud\/canonical-web-server/);
  assert.match(
    release,
    /REVOKER_IMAGE: ghcr\.io\/canonical-cloud\/canonical-session-revoker/,
  );
  assert.match(release, /\$\{\{ env\.WEB_IMAGE \}\}:\$\{\{ env\.RELEASE_SHA \}\}/);
  assert.match(release, /\$\{\{ env\.REVOKER_IMAGE \}\}:\$\{\{ env\.RELEASE_SHA \}\}/);
...
test("pinned app workflows cannot publish or declare a competing release", async () => {
  ...
  for (const name of (await readdir(appWorkflows)).filter((entry) => /\.ya?ml$/.test(entry))) {
...
test("application release boundary rejects known publication escape hatches", () => {
  ...
  const fixtures = [
    ...
    ["Docker build push flag", "run: docker buildx build --push -t ghcr.io/example/app:sha ."],
    ["OCI publishing tool", "run: oras cp source.example/app target.example/app"],
    ["OCI publishing tool", "run: crane append --base source --new_layer layer"],
    ["OCI publishing tool", "run: skopeo sync --src docker --dest docker images target"],
    ["alternate container push command", "run: buildah push app ghcr.io/example/app:sha"],
    [
      "outbound reusable workflow",
...
test("release is the monorepo's only privileged publishing workflow", async () => {
  ...
  for (const name of workflowFiles) {
    ...
    if (
      applicationPublisherSignals.some(([, pattern]) =>
        pattern.test(executableWorkflowText(workflow)),
      )
    ) {
