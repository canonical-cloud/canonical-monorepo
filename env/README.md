...
Secrets for `canonical-cloud/canonical-monorepo` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** Store a PEM as one quoted value containing escaped
  `\n` separators, for example: `JWT_PRIVATE_KEY="<single-line PEM with escaped newlines>"`.
...
```sh
just env-docker-run dev ghcr.io/canonical-cloud/canonical-monorepo:dev
